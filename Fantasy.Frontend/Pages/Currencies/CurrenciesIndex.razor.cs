...
public partial class CurrenciesIndex
{
    private List<Currency>? Currencies { get; set; }
    private MudTable<Currency> table = new();
    private readonly int[] pageSizeOptions = { 10, 25, 50, int.MaxValue };
    private int totalRecords = 0;
    private bool loading;
    private const string baseUrl = "api/currencies";
    ...
    private async Task<TableData<Currency>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
        ...
        var responseHttp = await Repository.GetAsync<List<Currency>>(url);
        if (responseHttp.Error)
        {
            var message = await responseHttp.GetErrorMessageAsync();
            Snackbar.Add(L[message!], Severity.Error);
            return new TableData<Currency> { Items = [], TotalItems = 0 };
        }
        if (responseHttp.Response == null)
        {
            return new TableData<Currency> { Items = [], TotalItems = 0 };
        }
        return new TableData<Currency>
        {
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = DialogService.Show<CurrencyEdit>($"{L["IUEdit"]} {L["Currency"]}", parameters, options);
        }
        else
        {
            dialog = DialogService.Show<CurrencyCreate>($"{L["IUNew"]} {L["Currency"]}", options);
        }
    ...
    private async Task DeleteAsync(Currency currency)
    {
        var parameters = new DialogParameters
            {
                { "Message", string.Format(L["IUDeleteConfirm"], L["Currency"], currency.Name) }
            };
